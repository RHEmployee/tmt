...
rlJournalStart
    ...
    rlPhaseStartTest "Filter by how"
        rlRun "tmt clean guests --dry -v --container | tee output"
        rlAssertGrep "Would stop guests in run $runid" "output"

        rlRun "tmt clean guests --dry -v --virtual | tee output"
        rlAssertNotGrep "Would stop guests in run $runid" "output"

        rlRun "tmt clean guests --dry -v --container --virtual | tee output"
        rlAssertGrep "Would stop guests in run $runid" "output"
